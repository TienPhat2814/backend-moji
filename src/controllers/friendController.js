...
export const sendFriendRequest = async (req,res) => {
    try {
        ...
        if(alreadyFriends) {
            return res.status(400).json({message: 'Hai người là bạn bè'})
        }
        ...
        return res.status(500).json({message: 'Lỗi hệ thộng'});
    }
};

export const accessFriendRequest = async (req,res) => {
    try {
        ...
        return res.status(200).json({
            message: "Chấp nhân lời mời kết bạn thành công",
            newFriend: {
                _id: from?._id,
